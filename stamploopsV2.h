...
if(nDCellIDs[0] & 1) {
	for(loopRng[0] = rangeFilteredCellIdsMin[0]; loopRng[0] <= rangeFilteredCellIdsMax[0]; loopRng[0]++) {
		if(loopRng[0] != nDCellIDs[0]) {
			...
			evaluateCellUnicompAdjacent(nCells, indexes, gridCellLookupArr, nNonEmptyCells, database, epsilon, index, indexLookupArr, point, cnt, pointIDKey, pointInDistVal, pointIdx, nDCellIDs, THREADPERPOINT, threadIdx.x % THREADPERPOINT);
		}
	}
}
#if NUMINDEXEDDIM>=2
